...
const ALL_PERMISSIONS = [
    ...
    { id: 'GALLERY_MANAGEMENT', label: 'Gallery Management' },
            ...
            <div style={{ maxWidth: '48rem', margin: '0 auto', padding: '1.5rem' }}>
                ...
                {selectedUser && (
                    <div className="modal-overlay" onClick={() => setSelectedUser(null)}>
                        <div className="modal-content" onClick={e => e.stopPropagation()} style={{ background: 'var(--color-card)', padding: '20px', borderRadius: '12px', maxWidth: '500px', width: '90%', border: '1px solid var(--color-border)', overflowY: 'auto', maxHeight: '90vh' }}>
                            <h3 style={{ color: 'var(--color-text)', marginBottom: '16px' }}>{isEditing ? 'User Details' : 'Add New Admin'}</h3>
                            ...
                            {isEditing && (
                                <div style={{
                                    ...
                                    <div style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--color-text-muted)', fontWeight: 700, letterSpacing: '0.05em', marginBottom: '8px' }}>Current Access</div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                        ...
                                        {selectedUser.role === 'POWER_USER' && (
                                            <span style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>
                                                ({selectedUser.permissions?.length || 0} permissions)
                                            </span>
                                        )}
                                    </div>
                                    {selectedUser.role === 'POWER_USER' && selectedUser.permissions?.length > 0 && (
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '8px' }}>
                                            {selectedUser.permissions.map(pId => {
                                                const label = ALL_PERMISSIONS.find(ap => ap.id === pId)?.label || pId;
                                                return (
                                                    <span key={pId} style={{ fontSize: '11px', color: 'var(--color-text-muted)', background: 'var(--color-background)', padding: '2px 6px', borderRadius: '4px' }}>
                                                        {label}
                                                    </span>
                                                );
                                            })}
                            ...
                            <div style={{ borderTop: isEditing ? '1px solid var(--color-border)' : 'none', paddingTop: isEditing ? '16px' : '0' }}>
                                <div style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--color-text-muted)', fontWeight: 700, letterSpacing: '0.05em', marginBottom: '12px' }}>
                                    {isEditing ? 'Modify Permissions' : 'User Information'}
                                </div>
