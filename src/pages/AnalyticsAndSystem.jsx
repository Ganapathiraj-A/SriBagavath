    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div style={{ background: 'var(--color-card)', padding: '16px', borderRadius: '16px', border: '1px solid var(--color-border)' }}>
                <div style={{ fontSize: '12px', color: 'var(--color-text-muted)', marginBottom: '4px' }}>Monthly Cost / Installed user</div>
                <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--color-text)' }}>₹{costPerInstalled.toFixed(4)}</div>
            </div>
            <div style={{ background: 'var(--color-card)', padding: '16px', borderRadius: '16px', border: '1px solid var(--color-border)' }}>
                <div style={{ fontSize: '12px', color: 'var(--color-text-muted)', marginBottom: '4px' }}>Monthly Cost / DAU</div>
...
const AnalyticsAndSystem = () => {
    ...
    // Derived Metrics (with backward compatibility)
    const lifetimeUsers = summary?.lifetimeTotalUsers || summary?.totalInstalledUsers || 0;
    const activeAudience = summary?.activeAudience28d || summary?.last28DaysUsers || 0;
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: 'var(--color-surface)', paddingBottom: '6rem' }}>
            <PageHeader
                ...
                >
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                            <div style={{ padding: '10px', background: 'var(--color-error-transparent)', borderRadius: '12px' }}>
                                <RefreshCcw size={20} color="var(--color-error)" />
                            </div>
                            <div>
                                <div style={{ fontWeight: '700', fontSize: '15px' }}>System Maintenance</div>
                                <div style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>Clear local cache & force resync</div>
                            </div>
                        </div>
                        <button 
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: '13px', color: 'var(--color-info-dark)', lineHeight: 1.5 }}>
                            <strong>Metric Note:</strong> 'Active Audience' matches Play Console more closely by counting unique users active in the last 30 days.
                        </div>
                    </div>
